// This file was auto-generated by Fern from our API Definition.

import type * as Corti from "../../api/index.js";
import * as core from "../../core/index.js";
import type * as serializers from "../index.js";

export const OAuthTokenRequest: core.serialization.ObjectSchema<
    serializers.OAuthTokenRequest.Raw,
    Corti.OAuthTokenRequest
> = core.serialization.object({
    clientId: core.serialization.property("client_id", core.serialization.string()),
    clientSecret: core.serialization.property("client_secret", core.serialization.string()),
    refreshToken: core.serialization.property("refresh_token", core.serialization.string().optional()),
});

export declare namespace OAuthTokenRequest {
    export interface Raw {
        client_id: string;
        client_secret: string;
        refresh_token?: string | null;
    }
}
