...
export type CustomStreamConnectArgs = { id: string } & Partial<Omit<StreamClient.ConnectArgs, "id" | "tenantName" | "token">>;
...
export class CustomStream extends StreamClient {
    public override async connect(args: CustomStreamConnectArgs): Promise<StreamSocket> {
        ...
        const token =
            authRequest.headers["Authorization"] ?? authRequest.headers["authorization"] ?? "";
